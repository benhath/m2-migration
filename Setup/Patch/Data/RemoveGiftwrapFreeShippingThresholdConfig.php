...
/**
 * Moves the giftwrap copy of the free delivery threshold onto Magento's free shipping carrier, then drops it.
 *
 * The shop has one threshold now, Magento's free shipping carrier
 * (Stores > Configuration > Sales > Delivery Methods > Free Shipping), read through
 * Ben\Shipping\Model\FreeShippingThreshold. The field is gone from the giftwrap section, so a saved row is
 ...
 */
class RemoveGiftwrapFreeShippingThresholdConfig implements DataPatchInterface
{
    ...
    // The path the giftwrap section used to save its own threshold to
    private const string CONFIG_XML_PATH_FREE_SHIPPING_THRESHOLD = 'giftwrap/settings/free_shipping_threshold';
    ...
    {
        return ['Ben\Giftwrap\Setup\Patch\Data\RemoveFreeShippingThresholdConfig'];
    }
    ...
    private function migrate(string $scope, int $scopeId): void
    {
        ...
        // The carrier's own number is the admin's later decision and outranks the copy being retired

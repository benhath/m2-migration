 ...
 * deploy. It is a Stores > Configuration > Promotion > Priority Access field now, kept encrypted like any other
 * credential. Nothing had ever been saved there, so without this a running shop would wake up with no password
 * at all and turn every visitor away. A shop that has already set its own is left alone, so this runs once and
 * anything typed in the admin afterwards stands.
 */
class SetPromotionPasswordSecret implements DataPatchInterface
{
    private const CONFIG_XML_PATH_PASSWORD_SECRET = 'promotion/password/secret';
